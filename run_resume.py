...
def main():
    ...
    parser.add_argument('--checkpoint_path', type=str, default='./outputs/latest_checkpoint.pt', help="Path to checkpoint file")
    ...
    config = Config(
        ...
        logging_steps=1,
        num_epochs_stage_one=0  # Set to 0 to skip stage one
    )
    ...
    try:
        ...
        total_steps = len(train_loader) * config.num_epochs_stage_two  # Only stage two steps
    ...
    try:
        ...
        logger.info(f"Resuming from global step {global_step} with best reward {best_reward}")
    except Exception as e:
        logger.critical(f"Error loading checkpoint: {e}")
        return
    ...
    try:
        ...
        logger.info("Trainer initialized successfully with loaded state.")
    except Exception as e:
    ...
    try:
        trainer.train()  # Will only run stage two since num_epochs_stage_one=0
